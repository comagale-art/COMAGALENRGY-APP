...
const AddTankPage: React.FC = () => {
  ...
  const { addTank, loading } = useTanks();
  ...
  const handleSubmit = async (data: any) => {
    try {
      ...
      setError('Erreur lors de l\'ajout de la citerne. Veuillez réessayer.');
    }
  };
  ...
  return (
    <Layout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Ajouter une citerne</h1>
        <p className="text-gray-600 dark:text-gray-400">
          Enregistrez une nouvelle citerne et ses opérations
        </p>
      </div>
      ...
      {success && (
        <Card className="mb-6 border-green-300 bg-green-50 dark:border-green-800 dark:bg-green-900/30">
          <div className="flex items-center text-green-700 dark:text-green-400">
            ...
            <p>Citerne ajoutée avec succès! Redirection...</p>
          </div>
        </Card>
      )}
