...
interface OrderMonthlyComparisonProps {
  orders: any[];
}
...
const OrderMonthlyComparison: React.FC<OrderMonthlyComparisonProps> = ({ orders }) => {
  ...
  const getMonthData = (month: number, year: number) => {
    ...
    return {
      ...
      quantity: monthOrders.reduce((sum, o) => sum + (o.quantity || 0), 0),
      revenue: monthOrders.reduce((sum, o) => sum + (o.totalPriceInclTax || 0), 0),
      avgOrderValue: monthOrders.length > 0
        ? monthOrders.reduce((sum, o) => sum + (o.totalPriceInclTax || 0), 0) / monthOrders.length
        : 0
    };
  };
  ...
  const chartData = {
    labels: ['Nombre de Commandes', 'Quantité (kg)', 'Revenu (DH)', 'Valeur Moy. Commande'],
    datasets: [
      {
        label: `${MONTHS[month1 - 1]} ${year1}`,
        data: [period1Data.count, period1Data.quantity, period1Data.revenue, period1Data.avgOrderValue],
      ...
      },
      {
        ...
        data: [period2Data.count, period2Data.quantity, period2Data.revenue, period2Data.avgOrderValue],
    ...
    <Card>
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center">
          <Calendar size={24} className="mr-2 text-comagal-blue dark:text-comagal-light-blue" />
          Comparaison Mensuelle des Commandes
        </h2>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Comparez les performances entre deux mois différents
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          <p className="text-sm text-gray-600 dark:text-gray-400">Revenu</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{period1Data.revenue.toFixed(0)} DH</p>
          <p className="text-sm text-green-600 dark:text-green-400">
            {calculateChange(period1Data.revenue, period2Data.revenue)}
          </p>
        </div>

        <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          <p className="text-sm text-gray-600 dark:text-gray-400">Valeur Moy.</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{period1Data.avgOrderValue.toFixed(0)} DH</p>
          <p className="text-sm text-green-600 dark:text-green-400">
            {calculateChange(period1Data.avgOrderValue, period2Data.avgOrderValue)}
          </p>
        </div>
      </div>
    </Card>
...
export default OrderMonthlyComparison;
